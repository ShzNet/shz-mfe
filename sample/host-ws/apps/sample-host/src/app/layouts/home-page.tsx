    ...
    <div className='flex min-h-svh flex-col bg-muted/30'>
      <header className='flex h-16 items-center justify-between border-b bg-background px-6'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            <p className='text-sm font-semibold tracking-tight'>Sample Host</p>
        ...
        <div className='grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]'>
              ...
              <div className='flex items-end gap-2 h-36'>
                {Array.from({ length: 7 }, (_, i) => (
                  <div key={i} className='flex flex-1 flex-col items-center gap-1.5'>
                    <div
                      className='animate-pulse rounded-t-sm bg-foreground/10 w-full'
                      style={{ height: `${50 + (i * 17) % 50}%` }}
                    />
          ...
          <Card>
            ...
            <CardContent className='space-y-3'>
              {apps.map((app) => (
                <Pulse key={app.id} className='h-16 w-full rounded-lg' />
              ))}
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className='flex min-h-svh flex-col bg-muted/30'>
      ...
      <main className='mx-auto flex w-full max-w-7xl flex-1 flex-col gap-4 p-4 md:p-6'>
        <div className='flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>Sample Host Dashboard</h1>
            <p className='text-sm text-muted-foreground'>Host scaffold generated by @shznet/nx-generators.</p>
          </div>
            ...
            <CardContent>
              <div className='flex items-end gap-2 h-36'>
                {activityChart.map((item) => (
                  <div key={item.day} className='flex flex-1 flex-col items-center gap-1.5'>
                    <div className='relative flex w-full flex-col justify-end gap-0.5' style={{ height: '112px' }}>
                      <div
              ...
              <div className='mt-3 flex items-center gap-4 text-xs text-muted-foreground'>
                <span className='flex items-center gap-1.5'>
                  <span className='inline-block size-2.5 rounded-sm bg-primary/70' />Requests
                </span>
                <span className='flex items-center gap-1.5'>
                  <span className='inline-block size-2.5 rounded-sm bg-destructive/60' />Errors
                </span>
              </div>
            </CardContent>
          </Card>
            ...
            <CardHeader>
              <div className='flex items-center justify-between gap-3'>
                <div>
                  ...
                  <CardDescription>Select an app to open its remote module.</CardDescription>
                </div>
